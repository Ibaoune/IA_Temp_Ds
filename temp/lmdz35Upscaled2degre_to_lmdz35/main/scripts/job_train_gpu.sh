...
#SBATCH --output=logs/out_gpu_%j.log
#SBATCH --error=logs/err_gpu_%j.log
#SBATCH --nodes=1
#SBATCH --time=24:00:00
#SBATCH --mem=128G
#SBATCH --gpus=1
#SBATCH --account=CLIMAT-UM6P-ST-IWRI-7KSIFKVWKUY-DEFAULT-GPU

# Ensure a config file is provided
if [ -z "$1" ]; then
    echo "Usage: sbatch job_train_gpu.sh <path_to_config.yaml>"
    exit 1
fi
