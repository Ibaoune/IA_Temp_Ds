...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute B-02 (bias of the 2nd percentile) using a metric-specific YAML config."
    )
    parser.add_argument(
        "metric_config",
        nargs="?",
...
def validate_metric_config(metric_cfg: dict):
    ...
    if metric_name != "b02":
        raise ValueError(f"metric.name must be 'b02', got {metric_name!r}")

    q = float(metric_cfg["metric"].get("quantile", 0.02))
    if not (0.0 < q < 1.0):
...
def compute_one_tag(
    ...
    p02_pred, p02_obs, b02_da = compute_quantile_fields(
        pred=pred_sub,
        obs=obs_sub,
        q=q,
        min_valid=min_valid,
    ...
    p02_pred.attrs["long_name"] = f"Predicted P02 temperature - {season_name}"
    p02_pred.attrs["units"] = "C"
    p02_pred.attrs["season"] = season_name
    p02_pred.attrs["quantile"] = q

    p02_obs.attrs["long_name"] = f"Observed P02 temperature - {season_name}"
    p02_obs.attrs["units"] = "C"
    p02_obs.attrs["season"] = season_name
    p02_obs.attrs["quantile"] = q

    b02_da.attrs["long_name"] = f"Bias of P02 temperature - {season_name}"
    b02_da.attrs["description"] = (
        f"B-02 computed as Q{q:.2f}(prediction) - Q{q:.2f}(observation) "
        f"for season '{season_name}'."
    )
    b02_da.attrs["units"] = "C"
    b02_da.attrs["season"] = season_name
    b02_da.attrs["quantile"] = q
...
def main():
    ...
    print("=== Temperature B-02 postprocessing ===")
    ...
    for season_name, months in selected_seasons.items():
        print(f"\n--- Computing B-02 for {season_name} ---")
        ...
        ds_out = xr.Dataset(
            {
                ...
                for var_name in ["p02_pred", "p02_obs", "b02"]
            },
            attrs=ds_out.attrs,
        )

        ds_out.attrs["metric"] = "b02"
        ...
        out_nc = data_dir / f"b02_{season_name.lower()}_mean_period.nc"

        if save_netcdf:
            ds_out.to_netcdf(out_nc)

        stats = spatial_summary(ds_out["b02"])
        stats_row = {
            "season": season_name,
            "metric": "b02",
            "quantile": q,
        ...
        if save_summary_json:
            save_json(
                stats_row,
                data_dir / f"summary_b02_{season_name.lower()}_mean_period.json",
            )
    ...
    if save_summary_csv_flag:
        save_summary_csv(
            summary_rows,
            data_dir / "b02_summary.csv",
        )
        print(f"\nSaved summary CSV: {data_dir / 'b02_summary.csv'}")

    print("\nAll B-02 products saved successfully.")
