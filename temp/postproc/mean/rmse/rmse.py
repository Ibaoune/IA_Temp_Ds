...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute temperature RMSE maps using a metric-specific YAML config."
    )
    parser.add_argument(
        "metric_config",
        nargs="?",
...
def validate_metric_config(metric_cfg: dict):
    ...
    if metric_name != "rmse":
        raise ValueError(f"metric.name must be 'rmse', got {metric_name!r}")
...
def compute_rmse_flagged(
    ...
    strategy : str
        - "daily_first":
            compute RMSE on paired daily values inside each group
        - "mean_first":
            compute sqrt((mean(model) - mean(obs))^2) inside each group
            Note: this behaves like an absolute difference of group means,
            not a classical temporal RMSE.
    ...
    if strategy == "daily_first":
        sq_err = ((model - obs) ** 2).where(valid == 1)
        grouped_rmse = np.sqrt(sq_err.groupby("group").mean("time", skipna=True))
    else:
        grouped_model_mean = model.groupby("group").mean("time", skipna=True)
        grouped_obs_mean = obs.groupby("group").mean("time", skipna=True)
        grouped_rmse = np.sqrt((grouped_model_mean - grouped_obs_mean) ** 2)

    grouped_rmse = grouped_rmse.where(valid_count >= min_valid)
    grouped_rmse = grouped_rmse.rename({"group": group_dim_name})
    grouped_rmse.name = "rmse"

    if return_by_year:
        return grouped_rmse

    return grouped_rmse.mean(dim=group_dim_name, skipna=True)
...
def compute_one_tag(
    ...
    """
    Compute one RMSE product for one tag:
    ...
    rmse_da.attrs["long_name"] = f"Temperature RMSE - {season_name}"
    rmse_da.attrs["description"] = (
        f"RMSE computed with strategy='{strategy}' for season '{season_name}'"
    )
    rmse_da.attrs["units"] = "C"
    rmse_da.attrs["season"] = season_name
    rmse_da.attrs["strategy"] = strategy
...
def main():
    ...
    print("=== Temperature RMSE postprocessing ===")
    ...
    for season_name, months in selected_seasons.items():
        ...
        ds_out = xr.Dataset({"rmse": rmse_da})
        ds_out.attrs["metric"] = "rmse"
        ...
        nc_name = f"rmse_{season_name.lower()}_{strategy}_{mode_suffix}.nc"
        ...
        if return_by_year:
            rmse_for_summary = rmse_da.mean(dim="year", skipna=True)
        else:
            rmse_for_summary = rmse_da

        stats = spatial_summary(rmse_for_summary)
        stats_row = {
            "season": season_name,
            "strategy": strategy,
            "mode": mode_suffix,
