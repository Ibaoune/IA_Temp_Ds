...
def _seasonal_mean_2d(da: xr.DataArray) -> xr.DataArray:
    maps = []
    template = da.isel(time=0, drop=True)

    for season in SEASONS:
        season_da = da.where(da["time"].dt.season == season, drop=True)
        if season_da.sizes.get("time", 0) == 0:
            mean_map = template * np.nan
        else:
            mean_map = season_da.mean(dim="time", skipna=True).load()
        maps.append(mean_map)

    return xr.concat(maps, dim=xr.IndexVariable("season", SEASONS))
...
def main() -> None:
    ...
    data_dir = get_metric_data_dir(settings, "seasonal_cycle")

    print("=== Computing seasonal cycle diagnostics ===")
    ...
    for source in sources:
        ...
        seasonal_mean = _seasonal_mean_2d(da)
        ref_seasonal_mean = _seasonal_mean_2d(ref)
        spatial_seasonal_mean = seasonal_mean.mean(dim=("lat", "lon"), skipna=True)

        if source["kind"] == "prediction":
            seasonal_bias = spatial_seasonal_mean - ref_seasonal_mean.mean(dim=("lat", "lon"), skipna=True)
            seasonal_rmse_clim = np.sqrt(
                ((seasonal_mean - ref_seasonal_mean) ** 2).mean(dim=("lat", "lon"), skipna=True)
            )
            data_vars[f"{key}_seasonal_bias"] = seasonal_bias
            data_vars[f"{key}_seasonal_rmse_clim"] = seasonal_rmse_clim
        else:
            seasonal_bias = xr.zeros_like(spatial_seasonal_mean)
            seasonal_rmse_clim = xr.zeros_like(spatial_seasonal_mean)

        data_vars[f"{key}_seasonal_mean"] = seasonal_mean
        data_vars[f"{key}_spatial_seasonal_mean"] = spatial_seasonal_mean
            ...
            rows.append(
                {
                    "model": label,
                    "season": season,
                    "mean_temperature": _as_float(spatial_seasonal_mean.sel(season=season)),
                    "bias": _as_float(seasonal_bias.sel(season=season)),
                    "rmse_clim": _as_float(seasonal_rmse_clim.sel(season=season)),
    ...
    ds_out.attrs["metric_name"] = "seasonal_cycle"
    ...
    out_nc = data_dir / (
        f"seasonal_cycle_bundle_{settings.selected_start}_{settings.selected_end}_{settings.output_tag}.nc"
    )
