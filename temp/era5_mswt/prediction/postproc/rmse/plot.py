...
def plot_rmse_map_panel(
    ...
    for label, ds in bundles:
        arr = ds["rmse_mean_period"].values
        lons = ds["rmse_mean_period"]["lon"].values
        lats = ds["rmse_mean_period"]["lat"].values
        arrays.append(
            apply_display_domain(
                arr,
                lons,
    ...
    levels = compute_sequential_levels(merged, n_bins=10, robust=robust, force_zero_min=True)
    ...
    plt.suptitle(
        f"Annual RMSE comparison ({title_tag})",
...
def plot_rmse_boxplot(
    ...
    ax.set_title(
        f"Distribution analysis: annual RMSE ({title_tag})",
        fontsize=18,
        fontweight="bold",
        pad=14,
    )
...
def main() -> None:
    ...
    data_dir, plot_dir = get_metric_dirs(settings, "rmse")
    bundles = load_metric_bundles(data_dir, settings, "rmse_bundle.nc", SELECTED_LABELS)

    print("=== Plotting temperature RMSE diagnostics ===")
